...
必须中文，严格 JSON。"""
    ...
    user=json.dumps({"schema":{"feasibility_score":"0-100 after product analysis","score_change_reason":"为什么升分/降分/保持","risk_level":"low|medium|high|critical","verdict":"continue|need_evidence|adjust_prd|pause","prd_reasonableness":{"verdict":"reasonable|needs_revision|weak","strengths":[],"problems":[],"missing_sections":[]},"product_plan":{"positioning":"产品定位","core_workflow":"核心流程","mvp_scope":[],"must_not_build":[]},"target_users":[{"segment":"目标用户分群","pain":"痛点","evidence":"证据/入口","confidence":"low|medium|high"}],"commercial_logic":{"why_now":"为什么现在需要","value_proposition":"价值主张","business_model":"商业模式","main_assumption":"最大假设"},"payment_path":{"buyer":"谁付费","trigger":"何时付费","pricing_test":"定价测试","conversion_path":"从入口到付费路径"},"competitor_table":[{"name":"竞品名","domain":"域名","url":"官网/证据链接","positioning":"定位","target_customer":"目标客户","core_features":["核心功能"],"pricing":"定价/收费信号","maturity":"low|medium|high","strengths":["做得好的方面"],"weaknesses":["弱项/缺口"],"direct_competition":"none|indirect|direct","similarity":"low|medium|high","threat":"low|medium|high","moat":"壁垒/护城河","acquisition":"主要获客方式/渠道信号","why_users_choose_them":"用户为什么会选它","why_users_leave":"用户为什么会不满意或离开","our_wedge":"我们可切入/绕开的突破口","evidence":"用于判断的证据摘要","confidence":"low|medium|high"}],"acquisition_channels":[{"channel":"获客方式","entry":"入口/关键词/社区","evidence":"证据","cost":"low|medium|high","confidence":"low|medium|high"}],"evidence_plan":[{"area":"需要验证的点","method":"抓取/搜索/访谈/落地页测试","success_signal":"成功信号"}],"wedge":"总体突破口/差异化入口；没有则说明没有","prd_gaps":[],"next_actions":[],"notification":"给用户的简短结论"},"prd":_short(p.prd_content),"original_opportunity":card_detail,"opportunity_group":group,"evidence_queries":queries,"competitors":comp_payload,"sitemap":sitemap_payload},ensure_ascii=False)
