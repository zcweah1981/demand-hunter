...
用户在机会推进详情页点击“上传 Markdown”，选择 `.md` 或 `.markdown` 文档。

规则：

- 前端只接受 Markdown / text 文件；
- 文件大小默认限制 1MB，避免误传超大文档；
- 上传后先读取到 PRD 编辑区，用户可检查和微调；
- 点击“保存 PRD”后才正式写入项目；
- PRD 内容至少 200 字，避免空文档触发无效验证。

### 页面编辑

用户也可以直接在机会推进详情页粘贴或编辑 PRD 内容。
