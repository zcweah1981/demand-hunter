...
export function FullPipelineForm(){
  ...
  async function submit(e:React.FormEvent){
    ...
    startTransition(async()=>{
      try{
        const data = await api<any>('/api/discovery/run',{method:'POST',body:JSON.stringify({seed,depth})})
        setSummary(data)
        router.refresh()
  ...
  return <form onSubmit={submit} className="mt-5 space-y-4">
    <div className="grid gap-3 md:grid-cols-[1fr_120px_auto]">
      ...
      <button className="btn" disabled={pending || !seed.trim()}>{pending?'Running pipeline...':'Run full pipeline'}</button>
    </div>
    {error&&<p className="rounded-xl border border-rose-500/40 bg-rose-500/10 p-3 text-sm text-rose-200">{error}</p>}
    {summary&&<div className="grid gap-3 text-sm md:grid-cols-4">
