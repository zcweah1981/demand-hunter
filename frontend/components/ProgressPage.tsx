...
export function ProgressPage({initialProjects, adoptedCards}:{initialProjects:any[]; adoptedCards:any[]}){
 ...
 async function create(cardId:number){setBusy(true); try{const r:any=await api(`/api/progress/from-card/${cardId}`,{method:'POST'}); setSelected(r); setPrd(r.project.prd_content||''); setUploadName(''); setUploadError(''); await refresh()}finally{setBusy(false)}}
 async function open(id:number){const r:any=await api(`/api/progress/${id}`); setSelected(r); setPrd(r.project.prd_content||''); setUploadName(''); setUploadError('')}
 async function savePrd(){if(!selected) return; setBusy(true); try{const r:any=await api(`/api/progress/${selected.project.id}/prd`,{method:'POST',body:JSON.stringify({content:prd})}); setSelected(r); await refresh()}finally{setBusy(false)}}
 ...
 async function uploadPrdFile(file:File|null){
  ...
  if(file.size>1024*1024){setUploadError('PRD 文件过大，请控制在 1MB 以内。'); return}
  const text=await file.text()
  if(text.trim().length<200){setUploadError('PRD 内容太短，至少需要 200 字后才能保存并启动验证。'); setPrd(text); setUploadName(name); return}
  setPrd(text)
  setUploadName(name)
 }
 ...
 return <div className="space-y-6">
  ...
  {selected&&<div className="fixed inset-0 z-50"><button className="absolute inset-0 bg-black/60" onClick={()=>setSelected(null)}/><aside className="absolute right-0 top-0 h-full w-full max-w-5xl overflow-y-auto border-l border-slate-800 bg-slate-950 p-5 shadow-2xl"><div className="mb-5 flex items-start justify-between gap-3"><div><div className="text-xs uppercase tracking-[0.25em] text-purple-300">Progress Project</div><h2 className="mt-1 text-2xl font-bold text-white">{selected.project.canonical_keyword}</h2><p className="mt-1 text-sm text-slate-400">状态 {selected.project.status} · 可行性 {Math.round(selected.project.feasibility_score||0)} · 风险 {selected.project.risk_level}</p></div><button className="btn-secondary" onClick={()=>setSelected(null)}>关闭</button></div><div className="grid gap-4 xl:grid-cols-2"><section className="rounded-2xl border border-slate-800 bg-slate-900/60 p-4"><div className="flex flex-wrap items-center justify-between gap-3"><div><h3 className="font-bold text-white">PRD.md</h3><p className="mt-1 text-xs text-slate-500">上传 Markdown 文档，或直接在下方编辑。保存后才会写入项目 PRD 文件。</p></div><div className="flex flex-wrap gap-2"><input ref={fileInputRef} type="file" accept=".md,.markdown,text/markdown,text/plain" className="hidden" onChange={e=>uploadPrdFile(e.target.files?.[0]||null)}/><button className="btn-secondary" disabled={busy} onClick={()=>fileInputRef.current?.click()}>上传 Markdown</button></div></div>{uploadName&&<p className="mt-3 rounded-xl border border-emerald-500/20 bg-emerald-950/30 px-3 py-2 text-xs text-emerald-200">已读取：{uploadName}。请检查内容后点击“保存 PRD”。</p>}{uploadError&&<p className="mt-3 rounded-xl border border-amber-500/20 bg-amber-950/30 px-3 py-2 text-xs text-amber-200">{uploadError}</p>}<textarea className="mt-3 h-96 w-full rounded-xl border border-slate-700 bg-slate-950 p-3 text-sm text-slate-200" placeholder="上传 PRD.md，或粘贴 / 编辑 PRD 内容。至少 200 字。" value={prd} onChange={e=>setPrd(e.target.value)}/><div className="mt-3 flex flex-wrap gap-2"><button className="btn" disabled={busy||prd.trim().length<200} onClick={savePrd}>保存 PRD</button><button className="btn-secondary" disabled={busy||!selected.project.prd_content} onClick={validate}>启动完整验证</button></div><p className="mt-2 text-xs text-slate-500">保存路径：{selected.project.prd_path||'保存后生成 prds/<slug>/PRD.md'} · 当前字数 {prd.trim().length}</p></section><section className="space-y-4"><Info title="下一步" text={selected.project.next_action}/><List title="竞品追踪" rows={selected.competitors.map((c:any)=>`${c.domain} · ${c.notes||''}`)}/><List title="策略建议" rows={selected.recommendations.map((r:any)=>`${r.title}: ${r.content}`)}/><List title="验证记录" rows={selected.runs.map((r:any)=>`#${r.id} ${r.status} · competitors=${r.summary?.competitors||0} · sitemap=${r.summary?.sitemap_snapshots||0}`)}/></section></div></aside></div>}
 </div>
}
