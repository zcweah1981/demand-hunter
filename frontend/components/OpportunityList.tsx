...
const SORTS:any={
 newest:'最新优先', oldest:'最早优先', score_desc:'分数高到低', score_asc:'分数低到高', verdict:'按判断分类'
}
const VERDICTS=['全部','Action','Watch','Reject','Block']

export function OpportunityList({cards, empty='暂无卡片', showVerdictFilter=true}:{cards:any[]; empty?:string; showVerdictFilter?:boolean}){
 ...
 return <>
  <div className="mb-3 flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-slate-800 bg-slate-950/70 p-3">
   ...
   <div className="flex flex-wrap gap-2">
    {showVerdictFilter&&<select className="input h-9 w-36 py-1 text-sm" value={verdict} onChange={e=>setVerdict(e.target.value)}>
     {VERDICTS.map(v=><option key={v} value={v}>{v==='全部'?'全部判断':verdictLabel(v)}</option>)}
    </select>}
    <select className="input h-9 w-36 py-1 text-sm" value={sort} onChange={e=>setSort(e.target.value)}>
     {Object.entries(SORTS).map(([k,v])=><option key={k} value={k}>{v as string}</option>)}
    </select>
   </div>
  </div>

  <div className="overflow-hidden rounded-3xl border border-slate-800 bg-slate-950/70">
   <div className="hidden grid-cols-[56px_120px_1.35fr_110px_80px_1fr_180px] gap-3 border-b border-slate-800 px-4 py-3 text-xs font-semibold text-slate-500 md:grid">
    <div>序号</div><div>日期</div><div>标题</div><div>判断</div><div>分数</div><div>摘要</div><div>复核</div>
   </div>
   {rows.length?rows.map((card,idx)=>{const biz=firstBusiness(card); return <div key={card.id} className="grid gap-3 border-b border-slate-800 px-4 py-4 last:border-b-0 md:grid-cols-[56px_120px_1.35fr_110px_80px_1fr_180px] md:items-center">
