 ...
 return <>
  <div className="mb-3 flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-slate-800 bg-slate-950/70 p-3">
   ...
   <div className="flex flex-wrap gap-2">
    {enableBulk&&<><button className="btn-secondary" onClick={()=>setSelectedIds(new Set(rows.map((r:any)=>r.id)))}>全选当前</button><button className="btn-secondary" onClick={()=>setSelectedIds(new Set())}>清空选择</button>{['Watch','Action','Adopted','Reject','Block'].map(x=><button key={x} className={feedbackButtonClass(x)} disabled={!selectedIds.size} onClick={()=>applyBulk(x)}>批量{x} {selectedIds.size?`(${selectedIds.size})`:''}</button>)}</>}
...
function InlineRowFeedback({onFeedback}:{onFeedback:(label:string)=>void}){const labels:any={Watch:'观察',Action:'行动',Adopted:'采纳',Reject:'拒绝',Block:'屏蔽'}; return <div className="flex flex-wrap gap-2">{['Watch','Action','Adopted','Reject','Block'].map(x=><button key={x} title={x} className={feedbackButtonClass(x)} onClick={()=>onFeedback(x)}>{labels[x]}</button>)}</div>}
