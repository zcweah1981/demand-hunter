...
export function SettingsForm({rows, initialGroup='search'}:{rows:any[]; initialGroup?:string}){
 ...
 async function providerHealth(){setTesting(true); setMsg(t('testing')); try{const h=await api<any>('/api/settings/provider-health',{method:'POST'}); setHealth(h); setMsg('✅ Provider health checked')}catch(e:any){setMsg(`❌ ${e.message}`)} finally{setTesting(false)}}
 ...
 return <div className="grid gap-6 lg:grid-cols-[300px_1fr]">
  ...
  <main className="space-y-5">
   <section className="rounded-3xl border border-blue-500/20 bg-gradient-to-br from-slate-950 to-blue-950/40 p-6 shadow-2xl">
    <div className="flex flex-wrap items-center justify-between gap-3"><div><div className="text-xs font-semibold uppercase tracking-[0.3em] text-blue-300">{t('configuration')}</div><h2 className="mt-2 text-3xl font-black">{group.title||t(group.titleKey)}</h2><p className="mt-1 text-sm text-slate-400">{group.desc||t(group.descKey)}</p></div><div className="flex gap-2">{active==='search'&&<><button className="btn-secondary" disabled={testing} onClick={test}>{testing?t('testing'):t('testProviders')}</button><button className="btn-secondary" disabled={testing} onClick={providerHealth}>{t('providerHealth')}</button></>}{group.keys.length>0&&<><span className={`badge ${dirty?'badge-watch':'badge-action'}`}>{saving?t('saving'):(dirty?t('unsaved'):t('savedState'))}</span><button className="btn" disabled={saving||!dirty} onClick={()=>saveGroup(group.keys)}>{t('saveGroup')}</button></>}</div></div>
    {msg&&<div className="mt-4 rounded-2xl border border-slate-800 bg-slate-950 p-3 text-sm text-slate-200">{msg}</div>}{health&&active==='search'&&<pre className="mt-4 overflow-x-auto rounded-2xl border border-slate-800 bg-slate-950 p-3 text-xs text-slate-300">{JSON.stringify(health,null,2)}</pre>}
   </section>
