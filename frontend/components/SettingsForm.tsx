...
const GROUPS:any[]=[
 ...
 {id:'boundaries',title:'边界与偏好',desc:'整个业务必须遵守的边界、偏好和高风险动作策略',keys:['BLOCKED_CATEGORIES','OPPORTUNITY_PREFERENCES','HIGH_RISK_ACTION_POLICY','BLOCKED_TERMS']},
 {id:'automation-cycle',title:'自动运行周期',desc:'统一周期驱动所有到期动作；后续再按需要拆专用周期',keys:['AUTO_RUN_ENABLED','AUTO_RUN_INTERVAL_MINUTES','AUTOMATION_CYCLE_MAX_SECONDS','AUTO_RUN_LIMIT','AUTOMATION_CYCLE_BUDGET_JSON','AUTOMATION_SOURCE_FLAGS_JSON','AUTOMATION_REPAIR_BEHAVIOR']},
 {id:'source-budget',title:'来源预算',desc:'按来源控制保守预算和 ROI 观察，不在这里直接删除数据',keys:['SOURCE_BUDGET_JSON','SERP_PROVIDER_ORDER','SERP_PROVIDER_ATTEMPT_LIMIT']},
...
const BOOL_KEYS=['AUTO_RUN_ENABLED','AUTO_RECHECK_ENABLED','REQUIRE_SOCIAL_FOR_ACTION','COLLECT_SOCIAL_EVIDENCE','FOUR_FIND_AUTO_ENABLED','FOUR_FIND_SERP_STRATEGY_ENABLED','FOUR_FIND_REWRITE_ON_SERP_REJECT']
const SECRET_KEYS=['SEARXNG_API_TOKEN','BRAVE_API_KEY','BRAVE_API_KEYS','TAVILY_API_KEY','TAVILY_API_KEYS','LLM_API_KEY','LLM_PRIMARY_API_KEY','LLM_FALLBACKS','BING_WEBMASTER_API_KEYS','DATAFORSEO_CREDENTIALS','SEMRUSH_API_KEYS','AHREFS_API_KEYS','SIMILARWEB_API_KEYS','SERPAPI_API_KEYS','ZENSERP_API_KEYS','SCALESERP_API_KEYS','YOUTUBE_API_KEYS','REDDIT_CREDENTIALS','PRODUCTHUNT_TOKENS','GITHUB_TOKENS','HUGGINGFACE_TOKENS','X_BEARER_TOKENS','WAPPALYZER_API_KEYS','BUILTWITH_API_KEYS']
const LIST_KEYS=['BRAVE_API_KEYS','TAVILY_API_KEYS','BING_WEBMASTER_API_KEYS','DATAFORSEO_CREDENTIALS','SEMRUSH_API_KEYS','AHREFS_API_KEYS','SIMILARWEB_API_KEYS','SERPAPI_API_KEYS','ZENSERP_API_KEYS','SCALESERP_API_KEYS','YOUTUBE_API_KEYS','REDDIT_CREDENTIALS','PRODUCTHUNT_TOKENS','GITHUB_TOKENS','HUGGINGFACE_TOKENS','X_BEARER_TOKENS','WAPPALYZER_API_KEYS','BUILTWITH_API_KEYS']
const MULTILINE_KEYS=['FOUR_FIND_AUTO_SEEDS','FOUR_FIND_AUTO_DOMAINS','BLOCKED_TERMS','BLOCKED_CATEGORIES','OPPORTUNITY_PREFERENCES','AUTOMATION_CYCLE_BUDGET_JSON','AUTOMATION_SOURCE_FLAGS_JSON','SOURCE_BUDGET_JSON']

const KEY_LABELS:Record<string,string>={
 SERP_PROVIDER_ORDER:'搜索源顺序', SERP_ROTATION_STRATEGY:'轮询策略', SERP_PROVIDER_ATTEMPT_LIMIT:'搜索源尝试次数', FOUR_FIND_SERP_STRATEGY_ENABLED:'启用 SERP 自动策略', FOUR_FIND_SERP_VARIANT_LIMIT:'SERP 查询变体数量',
 ...
 AUTO_RUN_ENABLED:'启用自动运行', AUTO_RUN_INTERVAL_MINUTES:'自动运行间隔（分钟）', AUTO_RUN_LIMIT:'每轮处理数量', AUTO_RECHECK_ENABLED:'复查老词', AUTO_RECHECK_LIMIT:'每轮老词复查数量', FOUR_FIND_AUTO_ENABLED:'启用四找闭环', FOUR_FIND_AUTO_SEEDS:'自动种子词', FOUR_FIND_AUTO_DOMAINS:'自动竞品域名', FOUR_FIND_IMPORT_LIMIT:'四找导入数量', FOUR_FIND_REWRITE_ON_SERP_REJECT:'SERP 失败后自动改写', FOUR_FIND_REWRITE_LIMIT:'改写恢复数量',
 MIN_ACTION_SCORE:'Action 最低分', REQUIRE_SOCIAL_FOR_ACTION:'Action 必须有社媒证据', COLLECT_SOCIAL_EVIDENCE:'采集社媒证据', BLOCKED_TERMS:'屏蔽词',
 AUTOMATION_CYCLE_MAX_SECONDS:'统一周期最长运行秒数', AUTOMATION_CYCLE_BUDGET_JSON:'每轮预算 JSON', AUTOMATION_SOURCE_FLAGS_JSON:'来源启用 JSON', AUTOMATION_REPAIR_BEHAVIOR:'异常修复策略',
 BLOCKED_CATEGORIES:'屏蔽分类', OPPORTUNITY_PREFERENCES:'机会偏好', HIGH_RISK_ACTION_POLICY:'高风险动作策略', SOURCE_BUDGET_JSON:'来源预算 JSON',
 BING_WEBMASTER_API_KEYS:'Bing Webmaster API Keys', DATAFORSEO_CREDENTIALS:'DataForSEO 凭证', SEMRUSH_API_KEYS:'Semrush API Keys', AHREFS_API_KEYS:'Ahrefs API Keys', SIMILARWEB_API_KEYS:'SimilarWeb API Keys', SERPAPI_API_KEYS:'SerpApi Keys', ZENSERP_API_KEYS:'Zenserp Keys', SCALESERP_API_KEYS:'Scale SERP Keys', YOUTUBE_API_KEYS:'YouTube Data API Keys', REDDIT_CREDENTIALS:'Reddit API 凭证', PRODUCTHUNT_TOKENS:'ProductHunt Tokens', GITHUB_TOKENS:'GitHub Tokens', HUGGINGFACE_TOKENS:'Hugging Face Tokens', X_BEARER_TOKENS:'X / Twitter Bearer Tokens', WAPPALYZER_API_KEYS:'Wappalyzer API Keys', BUILTWITH_API_KEYS:'BuiltWith API Keys',
}
...
export function SettingsForm({rows, initialGroup='search'}:{rows:any[]; initialGroup?:string}){
 ...
 return <div className="space-y-5">
   <section className="rounded-3xl border border-blue-500/20 bg-gradient-to-br from-slate-950 to-blue-950/40 p-6 shadow-2xl">
    <div className="flex flex-wrap items-center justify-between gap-3"><div><div className="text-xs font-semibold uppercase tracking-[0.3em] text-blue-300">{t('configuration')}</div><h2 className="mt-2 text-3xl font-black">{group.title||t(group.titleKey)}</h2><p className="mt-1 text-sm text-slate-400">{group.desc||t(group.descKey)}</p></div><div className="flex gap-2">{['search','searxng'].includes(active)&&<><button className="btn-secondary" disabled={testing} onClick={test}>{testing?t('testing'):t('testProviders')}</button></>}{!SPECIAL_SAVE_GROUPS.includes(active)&&group.keys.length>0&&<><span className={`badge ${dirty?'badge-watch':'badge-action'}`}>{saving?t('saving'):(dirty?t('unsaved'):t('savedState'))}</span><button className="btn" disabled={saving||!dirty} onClick={()=>saveGroup(group.keys)}>{t('saveGroup')}</button></>}</div></div>
