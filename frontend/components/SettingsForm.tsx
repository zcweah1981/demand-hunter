...
const GROUPS:any[]=[
 ...
 {id:'brave',title:'Brave',desc:'Brave 多 Key 轮询',keys:['BRAVE_API_KEYS']},
 {id:'tavily',title:'Tavily',desc:'Tavily 多 Key 轮询',keys:['TAVILY_API_KEYS']},
 {id:'llm',title:'LLM',desc:'Primary + fallback 模型配置',keys:['LLM_PRIMARY_PROVIDER','LLM_PRIMARY_MODEL','LLM_PRIMARY_API_KEY','LLM_FALLBACKS']},
...
export function SettingsForm({rows, initialGroup='search'}:{rows:any[]; initialGroup?:string}){
 ...
 async function saveGroup(keys:string[]){for(const k of keys){const s=settingFor(k); if(s.secret&&masked(s.value)) continue; await api('/api/settings',{method:'POST',body:JSON.stringify(s)})} setMsg(`Saved ${group.title}`)}
