...
export function SettingsForm({rows, initialGroup='search'}:{rows:any[]; initialGroup?:string}){
 ...
 async function changePassword(){setMsg('Changing password...'); try{await api('/api/auth/password',{method:'POST',body:JSON.stringify({current_password:currentPassword,new_password:newPassword})}); setCurrentPassword(''); setNewPassword(''); setMsg(`✅ ${t('passwordChanged')}`)}catch(e:any){setMsg(`❌ ${e.message}`)}}
