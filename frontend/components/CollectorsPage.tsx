...
type Section = 'overview' | 'flow' | 'conditions' | 'sources' | 'records'
...
const sectionItems:{id:Section; label:string; desc:string}[] = [
 ...
 {id:'flow', label:'发现流程', desc:'从搜索条件到机会'},
...
export function CollectorsPage({initialSection='overview'}:{initialSection?:string}){
 ...
 async function load(){
  try{
   const [h,seg,bg,rs,cs,repairs,repairAutos]=await Promise.all([
   ...
   setHealth(h); setSegments(seg); setBudget(bg); setRuns(rs); setCandidates(cs); setRepairRuns(repairs); setRepairAutoRuns(repairAutos)
 ...
 return <div className="space-y-6">
  ...
  <div className="grid gap-6 xl:grid-cols-[220px_1fr]"><aside className="rounded-3xl border border-slate-800 bg-slate-950/70 p-3 xl:sticky xl:top-4 xl:h-fit">{sectionItems.map(item=><Link key={item.id} href={`/collectors/${item.id}`} className={`mb-2 block rounded-2xl px-4 py-3 no-underline transition ${section===item.id?'bg-blue-600 text-white':'bg-slate-900/70 text-slate-300 hover:bg-slate-800'}`}><b>{item.label}</b><div className="mt-1 text-xs opacity-70">{item.desc}</div></Link>)}</aside>
   <main className="space-y-6">
    ...
    {section==='records'&&<Records records={visibleRecords} allRecords={records} sources={sources} sourceFilter={recordSource} statusFilter={recordStatus} setSourceFilter={setRecordSource} setStatusFilter={setRecordStatus} runDiscovery={runDiscovery} busy={busy}/>} 
   </main></div>
...
function TargetList({title,items}:{title:string;items:any[]}){return <section className="panel"><h2 className="text-xl font-bold">{title}</h2><div className="mt-4 space-y-2">{items.length?items.map((t:any)=><div key={t.id||t.value} className="rounded-xl bg-slate-950 p-3 text-sm"><b className="text-slate-100">{t.value}</b><span className="ml-2 text-slate-500">{t.target_type==='domain'?'网站':'搜索词'} · {segmentName(t.status)} · 优先级 {Math.round(t.priority||0)}</span></div>):<p className="text-sm text-slate-500">暂无</p>}</div></section>}
