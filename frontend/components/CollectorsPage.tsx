...
function cycleSummaryText(summary:any){const executed=summary?.executed??summary?.completed??0; const total=summary?.actions_collected??summary?.total??0; const failed=summary?.failed??0; return `执行 ${executed}/${total} 个动作，失败 ${failed} 个。`}
...
export function CollectorsPage({initialSection='overview'}:{initialSection?:string}){
 ...
 async function runDiscovery(){setBusy(true);setMsg('线索抓取动作已提交，顶部状态栏会显示运行进度...');try{await submitAction({action_type:'clue_model.run',target_type:'clue_model',target_id:'all',reason:'手动开始抓取机会线索',payload:{model:'all',limit:24}},false);const r=await automationCycleApi.run({include_default_actions:false, background:false});setMsg(`运行完成：${cycleSummaryText(r.summary||{})}`);await load()}catch(e:any){setMsg(`运行失败：${e.message}`)}finally{setBusy(false)}}
 async function refreshConditions(){setBusy(true);setMsg('搜索条件更新动作已提交，顶部状态栏会显示运行进度...');try{await submitAction({action_type:'collector.targets.refresh',target_type:'collector_targets',target_id:'all',reason:'手动从机会判断更新搜索条件'},false);const r:any=await automationCycleApi.run({include_default_actions:false, background:false});const result=r?.results?.[0]?.result?.raw_result||{};setMsg(`已更新：搜索词 ${result.keyword_targets||0}，网站 ${result.domain_targets||0}`);await load()}catch(e:any){setMsg(`更新失败：${e.message}`)}finally{setBusy(false)}}
 async function tidyConditions(){setBusy(true);setMsg('搜索条件整理动作已提交，顶部状态栏会显示运行进度...');try{await submitAction({action_type:'collector.targets.health',target_type:'collector_targets',target_id:'all',reason:'手动整理搜索条件健康状态'},false);const r:any=await automationCycleApi.run({include_default_actions:false, background:false});const result=r?.results?.[0]?.result?.raw_result||{};setMsg(`整理完成：暂停 ${result.cooled||0}，恢复 ${result.promoted||0}`);await load()}catch(e:any){setMsg(`整理失败：${e.message}`)}finally{setBusy(false)}}
...
function ManualRunPanel({model,onRun}:{model:any;onRun?:(run:any,effect:any)=>Promise<void>|void}){
 const [value,setValue]=useState('')
 const [busy,setBusy]=useState(false)
 const [msg,setMsg]=useState('')
 async function run(){
  if(!model.inputKind||!value.trim())return
  const items=value.split(/[\n,]+/).map(x=>x.trim()).filter(Boolean)
  if(!items.length)return
  const payload=model.inputKind==='domains'
   ? {model:model.id,domains:items,max_urls_per_domain:80,only_new:false,inputRefs:items.map((value:string)=>({type:'domain',value,label:value}))}
   : model.inputKind==='advanced'
    ? {model:model.id,roots:items,domains:[],days:30,limit_per_query:8,inputRefs:items.map((value:string)=>({type:'seed',value,label:value}))}
    : {model:model.id,seeds:items,limit_per_seed:10,inputRefs:items.map((value:string)=>({type:'seed',value,label:value}))}
  setBusy(true)
  setMsg('动作已提交，顶部状态栏会显示运行进度...')
  try{
   await submitAction({
    action_type:'clue_model.run',
    target_type:'clue_model',
    target_id:model.id,
    reason:`人工运行线索模型：${model.name}`,
    payload,
   }, false)
   const r:any=await automationCycleApi.run({include_default_actions:false, background:false})
   const summary=r.summary||{}
   setMsg(`运行完成：${cycleSummaryText(summary)}`)
   await onRun?.(null,null)
  }catch(e:any){
 ...
 if(!model.inputKind)return <div className="rounded-xl border border-slate-800 bg-slate-900 p-3"><b className="text-slate-200">人工入口</b><p className="mt-2 text-sm text-slate-500">{model.status==='planned'?'该模型尚未接入，暂不能人工运行。':'该模型当前没有独立人工运行入口。'}</p></div>
 return <div className="rounded-xl border border-slate-800 bg-slate-900 p-3"><div className="flex flex-wrap items-center justify-between gap-3"><b className="text-slate-200">人工入口</b><button className="btn" disabled={busy||!value.trim()} onClick={run}>{busy?'提交并运行中...':'提交运行'}</button></div><p className="mt-2 text-xs text-slate-500">输入 {model.inputKind==='domains'?'域名或 sitemap URL':'seed / 主题'}，多个值可换行或用逗号分隔。人工触发会进入统一动作队列，运行结果会写入模型运行记录和顶部状态栏。</p><textarea className="mt-3 min-h-24 w-full rounded-xl border border-slate-700 bg-slate-950 p-3 text-sm text-slate-100 outline-none focus:border-blue-500" value={value} onChange={e=>setValue(e.target.value)} placeholder={model.inputKind==='domains'?'example.com\nhttps://example.com/sitemap.xml':'ai tools\ninvoice automation'} />{msg&&<p className="mt-2 text-sm text-slate-400">{msg}</p>}</div>
}
