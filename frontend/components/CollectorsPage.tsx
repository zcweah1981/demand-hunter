...
function Overview({health,summary,latest,report,goodTargets,sourceRows}:any){return <div className="space-y-6">
 <section className="grid gap-4 md:grid-cols-5">
  ...
  <div className="card"><div className="kpi-label">有效搜索条件</div><b className="text-2xl text-emerald-300">{summary.usable_targets??0}</b><div className="mt-1 text-xs text-slate-500">可继续使用</div></div>
  <div className="card"><div className="kpi-label">待验证线索</div><b className="text-2xl text-blue-300">{summary.new_candidates??0}</b><div className="mt-1 text-xs text-slate-500">等待筛选验证</div></div>
  <div className="card"><div className="kpi-label">进入验证</div><b className="text-2xl text-purple-300">{latest.import?.imported??0}/{latest.import?.selected??0}</b><div className="mt-1 text-xs text-slate-500">本轮进入验证</div></div>
  <div className="card"><div className="kpi-label">需关注问题</div><b className="text-2xl text-amber-300">{health?.issues?.length||0}</b><div className="mt-1 text-xs text-slate-500">可能影响结果质量</div></div>
 </section>
 {report?.collector_audit&&<section className="panel"><div className="flex flex-wrap items-center justify-between gap-3"><div><h2 className="text-xl font-bold">最近一次抓取结论</h2><p className="mt-1 text-sm text-slate-400">只展示结果、来源和风险，不展示代码字段。</p></div><span className="badge">抓取 #{report.collector_audit.run_id}</span></div><div className="mt-4 grid gap-3 md:grid-cols-4"><MetricBlock label="进入验证" value={`${latest.import?.imported??0}/${latest.import?.selected??0}`} tone="purple"/><MetricBlock label="机会线索" value={(sourceRows||[]).reduce((a:any,r:any)=>a+Number(r.saved||0),0)} tone="green"/><MetricBlock label="抓取结果" value={(sourceRows||[]).reduce((a:any,r:any)=>a+Number(r.seen||0),0)||'-'} tone="blue"/><MetricBlock label="异常来源" value={(sourceRows||[]).filter((r:any)=>Number(r.errors||0)>0).length} tone="amber"/></div>{health?.issues?.length>0&&<div className="mt-4 rounded-2xl border border-amber-500/30 bg-amber-500/10 p-4 text-sm text-amber-100"><b>下一轮关注：</b>{health.issues.slice(0,3).map((x:any)=>x.text).join('；')}</div>}</section>}
...
function Conditions({seg,goodTargets,weakTargets,budget,busy,refreshTargets,applyTargetHealth}:any){return <div className="space-y-6"><section className="panel"><div className="flex flex-wrap items-center justify-between gap-3"><div><h2 className="text-xl font-bold">搜索条件分类</h2><p className="mt-1 text-sm text-slate-400">这些是系统继续找机会的依据。重点看哪些有效，哪些应该暂停。</p></div><div className="flex gap-2"><button className="btn-secondary" disabled={busy} onClick={applyTargetHealth}>整理搜索条件</button><button className="btn" disabled={busy} onClick={refreshTargets}>从机会判断刷新</button></div></div><div className="mt-4 grid gap-3 md:grid-cols-6">{[['winner','高价值'],['promising','可继续观察'],['new','新搜索条件'],['noisy','噪音高'],['cooldown','已暂停'],['exhausted','暂无价值']].map(([k,l])=><div key={k} className="rounded-xl bg-slate-950 p-3"><div className="text-xs text-slate-500">{l}</div><b className="text-2xl text-white">{seg[k]||0}</b></div>)}</div></section><section className="grid gap-4 xl:grid-cols-2"><TargetPreview title="有效搜索条件" items={goodTargets}/><TargetPreview title="降温/屏蔽条件" items={weakTargets}/></section><section className="panel"><h2 className="text-xl font-bold">下一轮投入</h2><div className="mt-4 grid gap-3 md:grid-cols-4">{(budget?.allocation||[]).map((row:any)=><div key={row.segment} className="rounded-2xl border border-slate-800 bg-slate-950 p-4"><div className="text-sm font-semibold text-slate-200">{row.label}</div><div className="mt-2 text-2xl font-black text-emerald-300">{row.budget}</div><div className="mt-1 text-xs text-slate-500">available {row.available}</div></div>)}</div></section></div>}
