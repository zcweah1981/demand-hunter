...
function TargetList({title,items}:{title:string;items:any[]}){return <section className="panel"><h2 className="text-xl font-bold">{title}</h2><div className="mt-4 space-y-2">{items.length?items.map((t:any)=><TargetRow key={t.id||t.value} t={t}/>):<p className="text-sm text-slate-500">暂无</p>}</div></section>}
function TargetRow({t}:{t:any}){const sources=t.source_effectiveness||[]; return <div className="rounded-xl bg-slate-950 p-3 text-sm"><div><b className="text-slate-100">{t.value}</b><span className="ml-2 text-slate-500">{t.target_type==='domain'?'网站':'搜索词'} · {segmentName(t.status)} · 优先级 {Math.round(t.priority||0)} · 成功/拒绝 {t.success_count||0}/{t.reject_count||0}</span></div>{sources.length>0&&<div className="mt-2 flex flex-wrap gap-2">{sources.slice(0,4).map((s:any)=><span key={s.source} className="rounded-lg bg-slate-900 px-2 py-1 text-xs text-slate-300">{sourceName(s.source)}：有效 {s.success||0} / 拒绝 {s.reject||0}</span>)}</div>}</div>}
