...
export default function Page(){
 ...
 useEffect(()=>{if(!data) return; const v:any={}; (data.fields||[]).forEach((f:any)=>v[f.name]=''); setValues(v); if(editIndex!==null){api<any>('/api/settings/secret/reveal',{method:'POST',body:JSON.stringify({key:data.setting_key,index:Number(editIndex)})}).then(r=>{if(!r.ok) throw new Error(r.error||'读取失败'); let raw=r.value||''; let parsed:any=null; try{parsed=JSON.parse(raw)}catch{}; const next:any={}; (data.fields||[]).forEach((f:any)=>next[f.name]=parsed?String(parsed[f.name]||''):raw); setValues(next)}).catch(e=>setMsg(`❌ ${e.message}`))}},[data?.id,editIndex])
 async function save(){
  setBusy(true); setMsg('')
  try{
   const endpoint=editIndex!==null?'/api/settings/api-keys/update':'/api/settings/api-keys'
   const body=editIndex!==null?{type_id:typeId,index:Number(editIndex),values}:{type_id:typeId,values}
   const r=await api<any>(endpoint,{method:'POST',body:JSON.stringify(body)})
   if(r.ok===false) throw new Error(r.error||'保存失败')
   setMsg(editIndex!==null?'✅ 已修改':'✅ 已保存')
 ...
 return <div className="mx-auto max-w-3xl space-y-6">
  <section className="rounded-3xl border border-blue-500/20 bg-gradient-to-br from-slate-950 to-blue-950/40 p-6 shadow-2xl">
   ...
   <h1 className="mt-4 text-3xl font-black">{editIndex!==null?'修改 Key':'新增 Key'}</h1>
   <p className="mt-2 text-sm text-slate-400">{editIndex!==null?'正在修改已保存凭证。保存后会覆盖原条目。':'先选择固定 Key 类型；如果是组合型凭证，会在同一页一次填完。'}</p>
  </section>
  <section className="rounded-3xl border border-slate-800 bg-slate-950/70 p-6 shadow-xl shadow-black/10">
   <label className="block"><span className="mb-1 block text-sm font-bold text-slate-300">Key 类型</span><select className="input w-full" value={typeId} disabled={editIndex!==null} onChange={e=>setTypeId(e.target.value)}>{types.map((t:any)=><option key={t.id} value={t.id}>{t.title} · {t.category}</option>)}</select></label>
   ...
   <div className="mt-6 flex flex-wrap gap-2"><button className="btn" disabled={busy||!typeId} onClick={save}>{busy?'保存中...':editIndex!==null?'保存修改':'保存'}</button><Link className="btn-secondary no-underline" href="/settings/api-keys">取消</Link></div>
