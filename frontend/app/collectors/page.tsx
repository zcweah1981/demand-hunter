  ...
  <div className="grid gap-6 xl:grid-cols-[220px_1fr]">
   <aside className="rounded-3xl border border-slate-800 bg-slate-950/70 p-3 xl:sticky xl:top-4 xl:h-fit">
    {[
     ...
     ['results','运行结果','历史产出'],
    ].map(([id,label,desc]:any)=><button key={id} onClick={()=>setTab(id)} className={`mb-2 block w-full rounded-2xl px-4 py-3 text-left transition ${tab===id?'bg-blue-600 text-white':'bg-slate-900/70 text-slate-300 hover:bg-slate-800'}`}><b>{label}</b><div className="mt-1 text-xs opacity-70">{desc}</div></button>)}
   </aside>
