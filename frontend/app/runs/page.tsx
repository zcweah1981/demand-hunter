...
export default async function Page(){
  ...
  return <div className="space-y-6">
    ...
    <section className="panel"><div className="flex flex-wrap justify-between gap-3"><div><h2 className="text-xl font-bold">最近运行摘要</h2><p className="mt-1 text-sm text-slate-500">runs={rows.length} · experiments={experiments.length} · repairs={repairs.length}</p></div><span className="badge">auto {auto?.enabled?'on':'off'}</span></div><div className="mt-4 space-y-4">{latestRuns.map(r=><div key={r.id} className="rounded-2xl border border-slate-800 bg-slate-950/70 p-4"><div className="flex justify-between"><b>#{r.id} {r.kind}</b><span className={r.status==='ok'?'badge badge-action':r.status==='running'?'badge':'badge badge-reject'}>{r.status}</span></div><div className="mt-2 text-xs text-slate-500">{r.started_at} → {r.finished_at||'running'}</div><FunnelSummary summary={r.summary}/></div>)}</div></section>
