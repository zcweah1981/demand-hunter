...
export default async function Page(){
  ...
  return <div className="space-y-6">
    ...
    <section className="panel"><div className="flex items-center justify-between gap-3"><h2 className="text-xl font-bold">自动实验</h2><span className="text-xs text-slate-500">最近 {experiments.length} 条</span></div><div className="mt-3 space-y-2">{experiments.length===0&&<p className="text-sm text-slate-500">暂无 experiment 记录。</p>}{experiments.map(x=>{const s=x.summary||{}; const e=x.effect||{}; const cls=e.status==='improved'?'text-emerald-300':e.status==='regressed'?'text-rose-300':e.status==='pending'?'text-amber-300':'text-slate-400'; return <div key={x.id} className="rounded-2xl border border-slate-800 bg-slate-950 p-3 text-xs"><div className="flex flex-wrap justify-between gap-2"><b className="text-slate-100">#{x.id} {s.action}</b><span className={cls}>{e.status||x.status}{typeof e.delta==='number'?` · Δ ${e.delta}`:''}</span></div><div className="mt-1 text-slate-500">repair #{s.repair_id} · {x.started_at}</div><p className="mt-2 text-slate-300">{e.recommendation||e.note||'等待实验后的 daily run 完成。'}</p></div>})}</div></section>
